...
# ── Paths ──────────────────────────────────────────────────────────────────────
PROJ_DIR="/home/wddkxg/Git/RTSP-Projs/RTSP-Player"
BUILD_DIR="$PROJ_DIR/build-linux"
BIN_DIR="$PROJ_DIR/bin/Release"
EXE_NAME="RTSP-Player"
...
do_deploy() {
    ...
    # 2. Copy FFmpeg shared libs (include all soname symlinks)
    ...
    for f in "$DIST_DIR/$EXE_NAME" "$DIST_DIR"/*.so*; do
        [ -f "$f" ] || continue
        # Skip symlinks (patchelf on symlink modifies the target, which is fine,
        # but we only need to process each real file once)
        [ -L "$f" ] && continue
